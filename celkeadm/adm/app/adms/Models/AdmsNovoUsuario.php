...
class AdmsNovoUsuario
{
    ...
    private function inserir()
    {
        ...
        $this->Dados['adms_niveis_acesso_id'] = $this->ser[0]['adms_niveis_acesso_id'];
        $this->Dados['adms_sits_usuario_id'] = $this->ser[0]['adms_sits_usuario_id'];
        $this->Dados['created'] = date('Y-m-d H:i:s');
        $cadUser = new \App\adms\Models\helper\AdmsCreate();
        $cadUser->exeCreate('adms_usuarios', $this->Dados);
        if ($cadUser->getResultado()) {
            if($this->ser[0]['env_email_conf'] == 1){
                $this->dadosEmail();
            }else{
    ...
    private function ser()
    {
        $ser = new \App\adms\Models\helper\AdmsRead();
        $ser->fullRead("SELECT env_email_conf, adms_niveis_acesso_id, adms_sits_usuario_id FROM adms_cads_usuarios WHERE id =:id LIMIT :limit", "id=1&limit=1");
        $this->ser = $ser->getResultado();
    }
