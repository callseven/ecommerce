 ...
 *
 * @copyright (c) year, Call Szpak - Celke
 */
class EditarCarousel
{
    ...
    public function editCarousel($DadosId = null)
    {
        ...
        if (!empty($this->DadosId)) {
            ...
            $_SESSION['msg'] = "<div class='alert alert-danger'>Erro: Carousel não encontrado!</div>";
            $UrlDestino = URL . 'carousel/listar';
    ...
    private function editCarouselPriv()
    {
        if (!empty($this->Dados['EditCarousel'])) {
            ...
            if ($editarCarousel->getResultado()) {
                ...
                $UrlDestino = URL . 'carousel/listar/' . $this->Dados['id'];
                header("Location: $UrlDestino");
            } else {
